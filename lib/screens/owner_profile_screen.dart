...
class _OwnerProfileScreenState extends State<OwnerProfileScreen> {
  ...
  Future<void> _loadProfileFromFirestore() async {
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        ...
        _referenceController.text = data['locationReference'] ?? '';
  ...
  Future<void> _searchAddress() async {
    ...
    try {
      final url = Uri.parse('https://maps.googleapis.com/maps/api/geocode/json?address=${Uri.encodeComponent(address)}&components=country:MX&key=$_googleApiKey');
      ...
      if (data['status'] == 'OK' && data['results'].isNotEmpty) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('📍 Ubicación actualizada'), backgroundColor: Colors.green));
        }
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('⚠️ ${data['error_message'] ?? 'Dirección no encontrada'}'), backgroundColor: Colors.red));
        }
      }
    } catch (e) {
      if (mounted) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(controller: _nameController, decoration: InputDecoration(labelText: 'Nombre Completo', prefixIcon: const Icon(Icons.person_outline, color: Colors.orange), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            const SizedBox(height: 16),
            TextField(controller: _phoneController, keyboardType: TextInputType.phone, decoration: InputDecoration(labelText: 'Teléfono de Contacto', prefixIcon: const Icon(Icons.phone_outlined, color: Colors.orange), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: IconButton(icon: _isSearching ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.search, color: Colors.orange), onPressed: _isSearching ? null : _searchAddress),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
              ),
            ),
            const SizedBox(height: 16),
            TextField(controller: _referenceController, maxLines: 2, decoration: InputDecoration(labelText: 'Referencia de Ubicación (Opcional)', prefixIcon: const Icon(Icons.note_alt_outlined, color: Colors.orange), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            const SizedBox(height: 8),
            ...
            Container(
              ...
              child: GoogleMap(
                ...
                markers: _markers,
                onTap: (latLng) { setState(() => _homeLocation = latLng); _updateMarker(); },
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isSaving ? const CircularProgressIndicator(color: Colors.white) : Text('Guardar Perfil', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
              ),
            ),
          ],
        ),
